  ...
  <div>
    ...
    <div style="width: 55%; margin: 50px auto">
      ...
      <div style="margin-top: 20px">
        <div style="display: flex; align-items: center; border-bottom: 1px solid #cccccc" v-for="item in data.cinemaData" :key="item.id">
...
const loadCinema = () => {
  request.get('/show/selectByFilmId/' + data.filmId).then(res => {
    if (res.code === '200') {
      ...
      data.cinemaData = Array.from(new Set(res.data.map(item => item.id)))
          .map(id => res.data.find(item => item.id === id));
    } else {
